...
export function GhostPanel({ ghost }: GhostPanelProps) {
  // Collapsed by default on mobile; `md:block` override shows it on desktop regardless
  const [collapsed, setCollapsed] = useState(true);

  return (
    <div className="panel overflow-hidden">
      {/* Header — acts as a toggle on mobile */}
      <button
        type="button"
        className="flex w-full items-center justify-between border-b border-crt/12 px-3 py-2 text-left"
      ...
      <div className={collapsed ? 'hidden md:block' : 'block'}>
        ...
        {/* Identity grid — compact on mobile: sigil inline with data */}
        <div className="grid gap-0 md:grid-cols-[80px_1fr]">
          ...
          {/* Data */}
          <div className="space-y-2 p-3 text-[0.95rem] leading-tight">
            <div>
        ...
        <div className="participant-label px-3 py-2">
        ...
        {/* Footer actions */}
        <div className="flex items-center justify-between px-3 py-1.5 text-[9px] uppercase tracking-[0.22em]">
